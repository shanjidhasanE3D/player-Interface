...
function processMsgForSocket(data) {
    let msg = JSON.parse(data);
    if (msg.type === 'config') {
        console.log("%c[Inbound SS (config)]", "background: lightblue; color: black", msg);
        // onConfig(msg);
...
function sendCloseMessage() {
    const msg = { "type": "shutDownStreamingApp" }
    writeClientMessage(JSON.stringify(msg))
}
async function writeClientMessage(message) {
    try {
        ...
        // Update the document with the new clientMsg field
        const fieldName = 'player_' + timestamp;
        const updateData = {};
        updateData[fieldName] = message;
        await docRef.update(updateData);

    } catch (error) {
        console.error('Error writing client message:', error);
    }
}
...
function setupWebRtcPlayer(htmlElement, config) {
    webRtcPlayerObj = new webRtcPlayer(config, setPlayerConnected);
    ...
    webRtcPlayerObj.onSFURecvDataChannelReady = function () {
        if (webRtcPlayerObj.sfu) {
            ...
            //writeClientMessage(JSON.stringify(requestMsg))
            ws.send(JSON.stringify(requestMsg));
        }
    }
            ...
            if (aggregatedStats.timestampStart) {
                if ((aggregatedStats.timestamp - aggregatedStats.timestampStart) > nextPrintDuration) {
                    if (ws && ws.readyState === WS_OPEN_STATE) {
                        ...
                        /*  writeClientMessage(JSON.stringify({
                             type: 'stats',
...
async function connect() {
    ...
    ws.send = function (message) {
        writeClientMessage(message); // Call your function with the message
    };
    ...
    ws.onmessage = function (event) {
        ...
        processMsgForSocket(event.data)
...
async function getInfo() {
    try {
        const response = await axios.get('https://ipgeolocation.abstractapi.com/v1/?api_key=667cb95019b04ea5beaa7c935dc5ce37');
        ...
        // Handle error (network failure, status code outside 2xx)
        console.error('Error:', error);
    }
}

// Function to insert a document into playersQ collection
async function insertPlayerDocument() {
    let previousData = {};
    // Regular expression to match the variable name and its value
    var appNameRegex = /\?(appName)=(.*?)(?=\?|$)/g;
    ...
    var ownerRegex = /\?(owner)=(.*?)(?=\?|$)/g;

    // Executing the regular expression and extracting variable names and values
    var ownerMatch;

    let owner = ""
    while ((ownerMatch = ownerRegex.exec(window.location.search)) !== null) {
        var variableName = ownerMatch[1];
            ...
            const unsubscribe = docRef.onSnapshot((snapshot) => {
                ...
                if (data !== undefined) {
                    if (data.Extraction !== undefined) {
                        console.log("Extraction Progress ==> " + data.Extraction)
                    }
                    if (data.Downloading !== undefined) {
                    ...
                    if (data.streamerLunched != undefined && data.streamerLunched === true && !streamerLoaded) {
                        ...
                        // Compare the current data with the previous data to find the changes
                        const addedFields = Object.keys(currentData).filter(key => !previousData.hasOwnProperty(key));
                        addedFields.forEach(async field => {
                            console.log(`New field "${field}" added with value:`, currentData[field]);
                            if (field.startsWith("streamer_")) {
                                processMsg(currentData[field])
                            }
                        });
                    ...
                    // Update the previous data for the next snapshot comparison
